...
char namen[12][10] =
  { "EINMAISH", " 1.RAST ", " 2.RAST ", " 3.RAST ", " 4.RAST ", " 5.RAST ",
      " 6.RAST ", " 7.RAST ", "ABMAISCH", " -PAUSE- ", " KOCHEN", " -PAUSE- " };
...
volatile int8_t start = 1, z, rast_c, heat = 0, r_stunde, r_minute,
    taster_a = 0, taster_b = 0, kochen;
...
void
init (void) //l�d die StandertWerte
{
...
//
uint8_t
get_key_press (uint8_t key_mask)
{
  cli();
...
void
TIMER0_interrupt_init (void) //konfiguriert die Uhr
{
  z = 0; //ISR-Z�hler = 0
...
void
uhrzeit_anzeigen (void) //zeigt die Zeit an
{
  char buffer[20];
  set_cursor (8, 1);
...
unsigned int
bestaetigt (char text_a[8], char text_b[12], char text_c[2])
{
  int a = 0;
  int b = 0;
  char answ[2][5] =
...
unsigned int
speicher_menu (char text[16], int platz) // platz==1 ist ohne "STANDART" / platz==0 ist mit "STANDART"
{
  char speicherplatz[5][12] =
    { " ", " SPEICHER-A", " SPEICHER-B", " SPEICHER-C", "  STANDART " };
  char speichern[2][12] =
    { "OFNE", "SPEI" };
  ...
  while (sp_menu == 0)
      ...
      else if (get_key_press (1 << KEY1))
	{
      ...
      else if (get_key_long (1 << KEY4))
	sp_menu = bestaetigt (speichern[platz], speicherplatz[sp_platz], "?");
    }
  return sp_platz;
}
//Ausgabe der Temperatur auf dem LCD
void
temp_ausgabe (unsigned int temp)
{
...
void
time_ausgabe (int stunde, int minute)
{
  char buffer_a[5];
  char buffer_b[5];
...
void
ubertrag (unsigned int b, unsigned int a, int lade)
{
  unsigned int x;
  int c = 0;
  int d = 0;
...
start_menu (void)
{
  ...
  int u_menu = 0;
  unsigned int offnen;
  char h_menu_name[14][15] =
    { "EINMAISCHEN", "  1.RAST   ", "  2.RAST   ", "  3.RAST   ", "  4.RAST   ",
	"  5.RAST   ", "  6.RAST   ", "  7.RAST   ", "ABMAISCHEN ", "PAUSE",
	"   KOCHEN  ", "TEMP-ABSCHALT", "TEMP-EINSCHALT" };
  char u_menu_name[2][10] =
    { "  TEMP:", "  TIME: " };
  int unter_menu[13] =
    { 0, 1, 1, 1, 1, 1, 1, 1, 0, 2, 1, 0, 0 };
...
//das Kochen
void
kochen_main (unsigned int temp_kochen)
{
