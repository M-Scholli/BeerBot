...
#include <string.h>
#include "uart.h"
unsigned int celsius_a;
int gag=0;
int uart=10;
//startet Kochen
void kochen_a(void)
{
kochen_main(celsius_a);
}

//Thermostat schaltet bei tmin_a die heizung an 
// und bei erreichen von tmax_a aus
void DS18x20_termostat_a(int tmin_a, int tmax_a)
...
{	
	...
	for (i=2;i<4-strlen(Buffer);i++) {
		//uart_puts_P("0");
		lcd_data (48);
	}
	celsius_c = cel_frac_bits*DS18X20_FRACCONV/100 ;
	celsius_b = cel*100;
	itoa(celsius_c, buffer,10);
if(celsius_c<=9 && celsius_c>0)
{lcd_string("0");}
lcd_string( buffer );
lcd_data (0b11011111);
lcd_string ("C");
celsius_a = celsius_c + celsius_b;
itoa(celsius_a,buffer,10);
if(uart==10)
{
	uart_puts(buffer);
	uart_putc('\r');      
